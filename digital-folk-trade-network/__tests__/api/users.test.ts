...
const mockAuth = require('@/lib/auth') as { requireAuthPayload: jest.Mock };
const mockRbac = require('@/lib/rbac') as { checkAccess: jest.Mock };
const mockRedis = require('@/lib/redis').redis as {
  get: jest.Mock;
  set: jest.Mock;
  del: jest.Mock;
};
...
describe('API /api/users', () => {
  ...
  describe('POST', () => {
    ...
    it('rejects when unauthorized', async () => {
      ...
      const res = await POST(new Request('http://localhost/api/users', { method: 'POST', body: JSON.stringify(baseBody) }));
      const body = await res.json();
    ...
    it('returns validation errors', async () => {
      ...
      const res = await POST(new Request('http://localhost/api/users', { method: 'POST', body: JSON.stringify({}) }));
      const body = await res.json();
    ...
    it('blocks potential SQL injection', async () => {
      ...
      expect(res.status).toBe(400);
      expect(body.error.code).toBe(ERROR_CODES.BAD_REQUEST);
      expect(body.error.details.input).toContain("' OR 1=1");
    });
